...
- rgba 표기법은 rgb에 불투명도 갑을 추가로 지정 (0은 완전 투명, 1은 완전 불투명)
...
```css
.container {
  flex-flow: flex-direction flex-wrap; 해당 순서로 옴
}
```
...
```css
.container {
  flex-basis: auto or 0 or 크기; /*특별히 지정하지 않으면 auto 기본*/
}
```
...
.item {
  flex: 1 1 auto;
  /* flex-grow: 1; flex-shrink: 1; felx-basis: auto; */

  flex: 1 500px;
  /* flex-grow: 1; flex-shrink: 1; felx-basis: 500px; */
...
<!-- ```css
gird-taemplate-colums: 200px 500px;
/* 첫번째는200,두번째는500으로맞춤 */
...
.item {
  ...
  grid-column-start: 1;
  grid-column-end: 4;
  /* grid-column 1/4 하나쓰는것이 grid-column-start 1, grid-column-end 4랑 각각 한개씩 쓰는 거와 같음 */
