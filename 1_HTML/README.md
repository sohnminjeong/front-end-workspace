...
- form 태그의 method 속성에 지정된 전송 방식으로 action 속성에 지정한 서버로 입력한 값들을 전송한다. <!--method에 올 수 있는 값 단 2가지 : get, post -->
...
- 한 줄 단위로 영역을 차지하는 요소 (너비가 100%) <!--자동 줄바꿈이 되는 이유-->
...
- 회사나 사이트에 대한 정보
- 저자권, 개인정보 보호정책, 연락처, 관리자 정보, 카피라이트 등
